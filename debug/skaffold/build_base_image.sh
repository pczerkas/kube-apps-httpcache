...
BUILDER_NAME=kube-apps-httpcache-base-builder
IMAGE_TAG=local/build-kube-apps-httpcache-base:latest

force=${1:-1}
if [ "$force" -eq 0 ]; then
  if docker inspect "$IMAGE_TAG" &>/dev/null; then
    echo "Image $IMAGE_TAG already exists, and not forced build"
    exit 0
  fi
fi
...
docker_buildx_build_default_args=(
  --progress plain
  -f debug/skaffold/build/package/docker/Dockerfile.base
  --tag "$IMAGE_TAG"
  --provenance false
  --load
)
