...
BUILDER_NAME=gcl-base-builder
IMAGE_TAG=local/gcl-ubuntu:latest
...
docker run \
  ...
  ghcr.io/bossm8/dockerfile-templater:debug \
    --verbose \
    --debug \
    --dockerfile.tpl build/gcl/Dockerfile.gcl-base.tpl \
    --variants.def debug/variants.yml \
    --dockerfile.tpldir debug/includes \
    --out.dir build/gcl \
    --out.fmt "Dockerfile.gcl-base"
...
docker_buildx_build_default_args=(
  --progress plain
  -f build/gcl/Dockerfile.gcl-base
  --tag "$IMAGE_TAG"
  --provenance false
  --load
)
