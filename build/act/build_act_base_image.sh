...
BUILDER_NAME=act-base-builder
IMAGE_TAG=local/act-ubuntu:latest
...
docker run \
  ...
  ghcr.io/bossm8/dockerfile-templater:debug \
    --verbose \
    --debug \
    --dockerfile.tpl build/act/Dockerfile.act-base.tpl \
    --variants.def debug/variants.yml \
    --dockerfile.tpldir debug/includes \
    --out.dir build/act \
    --out.fmt "Dockerfile.act-base"
...
docker_buildx_build_default_args=(
  --progress plain
  -f build/act/Dockerfile.act-base
  --tag "$IMAGE_TAG"
  --provenance false
  --load
)
