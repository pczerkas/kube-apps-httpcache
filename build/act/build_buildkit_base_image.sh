...
BUILDER_NAME=buildkit-base-builder
IMAGE_TAG=local/buildkit:latest
...
docker run \
  ...
  ghcr.io/bossm8/dockerfile-templater:debug \
    --verbose \
    --debug \
    --dockerfile.tpl build/act/Dockerfile.buildkit-base.tpl \
    --variants.def debug/variants.yml \
    --dockerfile.tpldir debug/includes \
    --out.dir build/act \
    --out.fmt "Dockerfile.buildkit-base"
...
docker_buildx_build_default_args=(
  --progress plain
  -f build/act/Dockerfile.buildkit-base \
  --tag "$IMAGE_TAG" \
  --provenance false
  --load
)
